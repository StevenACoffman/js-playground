...
if (!ENV.match(new RegExp(/prod|dev|test/))) {
  log(COLORS.red('Error: The argument \'env\' has incorrect value \'' + ENV +
    '\'! Usage: gulp test:e2e --env=(prod|dev|test)'));
  return process.exit(1);
}

if (!BROWSERS.match(new RegExp(/PhantomJS|Chrome|Firefox|Safari/))) {
  log(COLORS.red('Error: The argument \'browsers\' has incorrect value \'' + BROWSERS +
    '\'! Usage: gulp test:unit --browsers=(PhantomJS|Chrome|Firefox|Safari)'));
  return process.exit(1);
}
...
gulp.task('bundle', 'Create JS production bundle', ['jshint'], function (cb) {
  ...
  builder.loadConfig('./jspm.conf.js')
    .then(function () {
      builder.buildSFX('src/app/bootstrap', paths.tmp.scripts + 'build.js', {
          sourceMaps: true,
          config: {
            sourceRoot: paths.tmp.scripts
          }
        })
        .then(function () {
          return cb();
 ...
 */
gulp.task('extras',
  'Copy project files that haven\'t been copied by \'compile\' task e.g. (favicon, etc.) into the \'build/dist\' directory',
  function () {
 ...
 */
gulp.task('compile',
  'Does the same as \'jshint\', \'htmlhint\', \'images\', \'templates\' tasks but also compile all JS, CSS and HTML files', [
    'htmlhint', 'sass', 'bundle'
  ],
  function () {
    var projectHeader = $.header(banner);

    return gulp.src(paths.app.html)
      .pipe($.inject(gulp.src(paths.tmp.scripts + 'build.js', {
        read: false
      }), {
...
gulp.task('build', 'Build application for deployment', function (cb) {
  runSequence(
    ['clean'], ['compile', 'extras', 'images', 'fonts'],
    cb
  );
}, {
  options: {
